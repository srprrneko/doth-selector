...
public class AliasConvertUtil {
    ...
    public static String generateAliases(String originalSql) {
        // 通过正则from取主表别名                匹配主表别名                                  Pattern内部枚举表示忽略大小写
        ...
        // 改进: String[] joins = originalSql.split("(?i)\\s+JOIN\\s+"); // (?i): 忽略大小写; 匹配所有 JOIN部分
        String[] joins = originalSql.split("(?i)\\s+(?:(?:LEFT|RIGHT|INNER|OUTER)\\s+)?JOIN\\s+"); // 修改仅匹配join的缺点
        ...
        return originalSql.replaceFirst(
                ...
                "SELECT " + String.join(", ", processedFields) + " FROM"
        );
    }
