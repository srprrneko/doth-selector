...
@SupportedAnnotationTypes("com.doth.stupidrefframe.selector.v1.anno.CreateDaoImpl")
...
public class CreateDaoImplProcessorV1 extends AbstractProcessor {
    ...
    public synchronized void init(ProcessingEnvironment env) {
        ...
        四个关键工具：
          filer - 用于创建新的Java源文件
          messager - 用于向编译器输出警告/错误信息
          typeUtils - 类型/泛型 解析工具
          elementUtils - 包、类 解析工具
         */
        super.init(env);
        filer = env.getFiler();
    ...
    */
    ...
    private void generateEmptyImplementation(TypeElement abstractClass) {
        ...
        // 获取父类所在包名（自动继承包结构）
        // 示例：若abstractClass是com.example.UserDao，则返回com.example包元素
        // getQualifiedName(): 完全限定名（包含包路径的全名）
        ...
        try (Writer writer = filer.createSourceFile(packageName + "." + className).openWriter()) {
            ...
            for (Element enclosedElement : abstractClass.getEnclosedElements()) {
                // 对抽象方法处理, 是否为方法 并且, 访问修饰符是否是抽象的, 例: private int count; 字段，不会被选中; 不带抽象关键字的也不会被选中
    ...
    private void generateMethodImpl(Writer writer, ExecutableElement method, TypeElement entityType) throws Exception {
        ...
        // 参数列表处理（示例：String name, int age）
        // param.asType()：获取参数的类型镜像 -> java.lang.String
        // param.getSimpleName()：获取参数名称 -> name, age...
        ...
        if (methodName.startsWith("queryBy")) {
            ...
            List<String> conditionParts = splitConditions(byPart);

            if (params.size() != conditionParts.size()) {
                messager.printMessage(Diagnostic.Kind.ERROR,
                        "参数数量与条件数量不匹配 期望: " + conditionParts.size() + " 实际: " + params.size(),
                        method);
            ...
            for (int i = 0; i < conditionParts.size(); i++) {
                String conditionPart = conditionParts.get(i);
     ...
     * 分割条件
     * @param input 字符串
     * @return 条件list, 便于 join 操作
     */
    private List<String> splitConditions(String input) {
        List<String> parts = new ArrayList<>();
        String[] splits = input.split("With|Vz|And");
